...
export function executeWpCli(command, site) {
  return new Promise((resolve, reject) => {
    ...
    if (site.type === 'remote' && site.ssh_host) {
      ...
      if (site.ssh_password) {
        // Use sshpass for password-based auth
        fullCommand = `sshpass -p '${site.ssh_password.replace(/'/g, "'\\''")}' ssh -o StrictHostKeyChecking=no ${sshTarget} "${command}${remotePath}"`;
      } else {
    ...
    exec(fullCommand, { timeout: TIMEOUT_MS, maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => {
      // Filter PHP deprecation/warning noise from output
      const filterNoise = (text) => text.split('\n').filter(line =>
        !line.match(/^(PHP )?(Deprecated|Warning|Notice|Parse error|Fatal error):/i) &&
        !line.match(/^\s*$/) &&
        !line.includes('Implicitly marking parameter') &&
        !line.includes('phar://') &&
        !line.includes('eval()\'d code')
      ).join('\n').trim();

      if (error) {
        const output = filterNoise(stderr || error.message);
        resolve({ success: false, output: output || error.message.trim(), command: fullCommand });
        return;
      }

      let parsed = filterNoise(stdout);
