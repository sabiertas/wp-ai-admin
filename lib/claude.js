...
import { mcpToolsToClaude, parseMcpToolName, executeMcpTool, getMcpSummary } from './mcp-proxy.js';
 ...
 */
function buildSystemPrompt(messages, site) {
...
export async function chat(messages, site) {
  ...
  const toolResults = [];
  const systemPrompt = buildSystemPrompt(messages, site);
